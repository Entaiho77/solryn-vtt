...
**Phase D — the board (first increment):** HTML5 Canvas board (map placed at top-left, fixed grid + toggle, no stretch), real-time map/token/fog state, the role-configured edge-bar/drawer shell, GM map tools (upload→data-URL maps with type/scale, fog of war with cover/clear/click-paint, add-creature from the bestiary or a quick build), tokens with snap-to-grid drag and the permission matrix (control follows ownership; players see monsters as name+image only; hidden tokens), the floating token card (GM HP/damage/hide/defeat controls), and the player board view (character quick-view drawer + full-sheet overlay, Dice and Rules-reference drawers).

**Next phases:** D part 2 — measure tool, traps lifecycle, saved "my creatures", Firebase Storage for map images. E — initiative tracker, harvest/loot, chat + whispers, notes, level-up ceremony. See [`docs/Solryn_VTT_Build_Brief.md`](./docs/Solryn_VTT_Build_Brief.md).
